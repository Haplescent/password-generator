...
function generatePassword() {
  ...
  let password_length = parseInt(sliderBar.value);
  // character types to include in the password choose lowercase, uppercase, numeric, and/or special characters
  let lowercase = document.querySelector("#lowercase");
  let lowercaseCharSet = "qwertyuiopasdfghjklzxcvbnm";

  let uppercase = document.querySelector("#uppercase");
  let uppercaseCharSet = lowercaseCharSet.toUpperCase();

  let numeric = document.querySelector("#numeric");
  let numericCharSet = "1234567890";

  let specialChar = document.querySelector("#specialChar");
  let SpecialCharSet = "!@#$%^&*()-_=+[{]}|;:'<>,.?";

  // initiating function scoped varibles
  let passwordCharSet = "";
  let returnPassword = "";
  let boxesChecked = 0;

  // Logic to add attributes to passwordCharSet
  ...
  let passwordCharSetLength = passwordCharSet.length;

  for (let i = 0; i < password_length; i++) {
    returnPassword += passwordCharSet.charAt(
      Math.floor(Math.random() * passwordCharSetLength)
    );
  }
...
!(function (a) {
  ...
  (j.prototype.init = function () {
    ...
    (j.prototype.setValue = function (a) {
      a !== this.value &&
        this.$element.val(a).trigger("input", {
          origin: this.identifier,
        });
    }),
